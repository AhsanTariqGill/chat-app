module SessionsHelper
    ...
    def get_conversation(other_user)
    	@conversation = Conversation.find_by(user1: current_user.id, user2: other_user.id) || Conversation.find_by(user2: current_user.id, user1: other_user.id)
    end
    ...
    def get_request(other_user)
    	@request = Request.find_by(sender_id: current_user.id, receiver_id: other_user.id) || Request.find_by(sender_id: current_user.id, receiver_id: other_user.id)
    end
